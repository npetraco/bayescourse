...
stan.code   <- paste(readLines(system.file("stan/exp-trunc-cauchy.stan", package = "bayesutils")),collapse='\n')
...
dat   <- list(
  ...
  "t"     = times,
  "n"     = length(times),
  "mu"    = 0.1,   # Hyper param for stud.T prior
  "sigma" = 8,     # Hyper param for stud.T prior
  "nu"    = 3      # Hyper param for stud.T prior
)
